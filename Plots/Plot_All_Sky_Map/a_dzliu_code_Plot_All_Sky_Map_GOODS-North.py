...
if __name__ == "__main__":
    ...
    data = asciitable.read('datatables/irac_mips_fluxes_hdfn.dat')
    print(data)
    data_ra = data['col1']
    data_dec = data['col2']
    ...
    ax.annotate('GOODS-North (enlarged)', xy=(0.001, 1.001), color='#000000', ha='left', va='bottom', xycoords=ax.transAxes, zorder=10, fontsize=18, fontweight='bold')
    plt.subplots_adjust(left=0.1, bottom=0.1, right=0.9, top=0.9)
    fig.savefig('Plot_All_Sky_Map_GOODS-North.pdf')
    #plt.show(block=True)
    
    
    
    hdu = fits.open('photos/GOODSN_Photos/n_mips_1_s1_v0.37_sci_BS.null.fits.gz')[0]
    wcs = WCS(hdu.header)
    ...
    ax2.plot(data2_px, data2_py, '.', ms=1.5, c='#ff8d1e')
    ...
    ax2.invert_xaxis()
    ax2.annotate('GOODS-North', xy=(0.01, 0.99), color='#000000', ha='left', va='top', xycoords=ax2.transAxes, zorder=10, fontsize=15, fontweight='bold')
    ...
    fig2.savefig('Plot_Field_GOODS-North.pdf')
    #plt.show(block=True)
