...
if __name__ == "__main__":
    ...
    data2 = asciitable.read('datatables/S2COSMOS-850_20160313_source.cat')
    data2_ra = data2['RA_deg']
    data2_dec = data2['DEC_deg']
    
    
    #data = asciitable.read('datatables/irac_mips_fluxes_cdfs.dat')
    #print(data)
    #data_ra = data['col1']
    #data_dec = data['col2']
    
    
    hdu = fits.open('datatables/datatable_ID_RA_DEC_COSMOS_master_catalog_201704.fits')[1]
    data = hdu.data
    data_ra = data['RA']
    data_dec = data['Dec']
    ...
    #ax.plot(xs, ys, 'o', ms=1.5) # here we do not plot each data point but use hexbin
    ax.hexbin(xs, ys, gridsize=(5,5)) # https://python-graph-gallery.com/84-hexbin-plot-with-matplotlib/
    ax.annotate('COSMOS', xy=(0.001, 0.99), color='#000000', ha='left', va='top', xycoords=ax.transAxes, zorder=10, fontsize=15, fontweight='bold')
    plt.subplots_adjust(left=0.1, bottom=0.1, right=0.9, top=0.9)
    fig.savefig('Plot_All_Sky_Map.pdf')
    #plt.show(block=True)
    
    
    
    hdu = fits.open('photos/COSMOS_Photos/mips_24_GO3_sci_10.fits')[0]
    wcs = WCS(hdu.header)
