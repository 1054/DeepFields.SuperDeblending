...
if __name__ == "__main__":
    ...
    data = Table.read('datatables/UDS_Catalogs/Galametz_2013_Table3_UDS_Catalog.fits.gz')
    print(data)
    data_ra = data['RAdeg']
    data_dec = data['DEdeg']
    ...
    ax.annotate('UKIDSS Ultra-Deep Survey (enlarged, original 0.8 sq. degrees)', xy=(0.001, 1.001), color='#000000', ha='left', va='bottom', xycoords=ax.transAxes, zorder=10, fontsize=18, fontweight='bold')
    plt.subplots_adjust(left=0.1, bottom=0.1, right=0.9, top=0.9)
    fig.savefig('Plot_All_Sky_Map_UDS.pdf')
    #plt.show(block=True)
    
    
    
    hdu = fits.open('photos/UDS_Photos/mips_mosaic.null.fits.gz')[0]
    wcs = WCS(hdu.header)
    ...
    ax2.invert_xaxis()
    ax2.annotate('UDS', xy=(0.01, 0.99), color='#000000', ha='left', va='top', xycoords=ax2.transAxes, zorder=10, fontsize=15, fontweight='bold')
    ax2.annotate('Spitzer/MIPS', xy=(0.99, 0.99), color='#1e90ff', ha='right', va='top', xycoords=ax2.transAxes, zorder=10, fontsize=15, fontweight='bold')
    ...
    fig2.savefig('Plot_Field_UDS.pdf')
    #plt.show(block=True)
