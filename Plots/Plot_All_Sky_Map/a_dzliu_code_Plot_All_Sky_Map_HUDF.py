...
if __name__ == "__main__":
    ...
    data = asciitable.read('datatables/HUDF_Catalogs/Dunlop_2017_Table_2_HUDF_ALMA_1p3mm_Catalog.txt')
    print(data)
    data_ra = data['RA_ALMA']
    data_dec = data['Dec_ALMA']
    ...
    ax.annotate('HUDF (enlarged, original 0.45 sq. degrees)', xy=(0.001, 1.001), color='#000000', ha='left', va='bottom', xycoords=ax.transAxes, zorder=10, fontsize=18, fontweight='bold')
    plt.subplots_adjust(left=0.1, bottom=0.1, right=0.9, top=0.9)
    fig.savefig('Plot_All_Sky_Map_HUDF.pdf')
    #plt.show(block=True)
    
    
    
    hdu = fits.open('photos/HUDF_Photos/hlsp_hudf09_hst_wfc3ir_hudf09_F160W_v1_sci.null.fits.gz')[0]
    wcs = WCS(hdu.header)
    ...
    ax2.plot(data_px, data_py, '.', ms=0.5, c='#1e90ff')
    ...
    ax2.invert_xaxis()
    ax2.annotate('HUDF', xy=(0.01, 0.99), color='#000000', ha='left', va='top', xycoords=ax2.transAxes, zorder=10, fontsize=15, fontweight='bold')
    ax2.annotate('HST/WFC3 F160W', xy=(0.99, 0.99), color='#1e90ff', ha='right', va='top', xycoords=ax2.transAxes, zorder=10, fontsize=15, fontweight='bold')
    ax2.annotate('ALMA 1.3mm', xy=(0.99, 0.94), color='#ff8d1e', ha='right', va='top', xycoords=ax2.transAxes, zorder=10, fontsize=15, fontweight='bold')
    ...
    fig2.savefig('Plot_Field_HUDF.pdf')
    #plt.show(block=True)
