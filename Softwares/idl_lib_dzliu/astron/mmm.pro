...
;-
 ...
 mxiter = 60              ;Maximum number of iterations allowed   ;<corrected><20130913><DzLiu> for some convolved images 60 is better
...
; Select the pixels between Cut1 and Cut2
 ...
 sum = total(delta,/double,/NAN) ;[DzLiu] ",/NAN"
 sumsq = total(delta^2,/double,/NAN) ;[DzLiu] ",/NAN"
...
; Compute Chauvenet rejection criterion.
    ...
    r = alog10( double( maximm-minimm ) ) ; [DzLiu] "float"->"double"
...
; at both ends of the interval of acceptable values.
    ...
    if not done then begin
        ...
        sum = sum - istep*total(delta,/double,/NAN)
        sumsq = sumsq - istep*total(delta^2,/double,/NAN)
        redo = 1b
        minimm = newmin
     endif
;       
...
;
   ...
   skymn = sum/nsky       
   sigma = double( sqrt( (sumsq/nsky - skymn^2)>0 )) ; [DzLiu] "float"->"double"
   skymn = skymn + skymid 
...
;  central value by at least 0.25 times the read noise.
   ...
   skymed = total(sky[j:k], /NAN)/(k-j+1) ; [DzLiu] ", /NAN"
...
;       
 skew = double( (skymn-skymod)/max([1.,sigma]) ) ; [DzLiu] "float"->"double"
 nsky = maximm - minimm 
