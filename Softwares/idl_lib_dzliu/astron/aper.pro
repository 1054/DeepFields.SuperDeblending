pro aper,image,xc,yc,mags,errap,sky,skyerr,phpadu,apr,skyrad,badpix, $
       ...
       NoRandomPhotonNoise=NoRandomPhotonNoise
;+
; NAME:
;      APER
; PURPOSE:
...
;             Set parameter limits
 ...
 maxsky = 990000  ; Maximum number of pixels allowed in the sky annulus. <Modified><DzLIU>
;                                
...
DONE: 
 ...
 if N_elements(SETSKYVAL) GT 0 then begin
     ...
     if N_elements( SETSKYVAL ) NE 3 then message, 'ERROR - Keyword SETSKYVAL must contain 1 or 3 elements'
     skyrad = [ 0., max(apr) + 1] ; <Note><20131025><DzLiu> APER will change the skyrad if SETSKYVAL is set!
 endif
...
;Get radii of sky annulii
 ...
 endif else skyrad = double(skyrad)
 ...
 if keyword_set(flux) then $
          fmt = '(D16.6,1x,A," ",D-14.6)' else $           ;Flux format
          fmt = '(F9.3,A,F5.3)'                  ;Magnitude format
 fmt2 = '(I5,2F8.2,F7.2,3A,3(/,28x,4A,:))'       ;Screen format
 fmt3 = '(I4,5F12.4,6A,2(/,44x,9A,:))'           ;Print format

 mags = dblarr( Naper, Nstars) & errap = mags           ;Declare arrays
 sky = dblarr( Nstars )        & skyerr = sky     
 area = !PI*apr*apr                 ;Area of each aperture

 if keyword_set(EXACT) then begin
      bigrad = apr + 0.5d
      smallrad = apr/sqrt(2) - 0.5d
 endif
 ...
 if keyword_set(PRINT) then begin      ;Open output file and write header info?
   if size(PRINT,/TNAME) NE 'STRING'  then file = 'aper.prt' else file = print
   if not keyword_set(SILENT) then message,'Results will be written to a file ' + file,/INF
   openw,lun,file,/GET_LUN
   printf,lun,'Program: APER: '+ systime(), '   User: ', $
      getenv('USER'),'  Host: ',getenv('HOST')
   for j = 0, Naper-1 do printf,lun, $
               format='(a,i2,a,f12.4)','Radius of aperture ',j,' = ',apr[j]
   if N_elements(SETSKYVAL) EQ 0  then begin
   printf,lun,f='(/a,f12.4)','Inner radius for sky annulus = ',skyrad[0]
   printf,lun,f='(a,f12.4)', 'Outer radius for sky annulus = ',skyrad[1]
...
;
  ...
  if ( Nbad GT 0 ) then message, /INF, 'WARNING - ' + strn(nbad) + ' star positions outside image'
  if keyword_set(flux) then begin 
          badval = !VALUES.F_NAN
	        baderr = badval
  endif else begin 
...
;         the distance of each pixel to the center pixel.
  ...
  if keyword_set(exact) then begin 
      ...
      r = sqrt(rsq) - 0.5d    ;2-d array of the radius of each pixel in the subarray
  endelse
...
;       below BADLO threshold.  SKYBUF will be 1-d array of sky pixels
  ...
  else if chk_badpix then skypix = skypix and ( rotbuf GT badpix[0] ) and (rotbuf LT badpix[1] )
  ...
  if ( nsky LT minsky ) then begin                       ;Sufficient sky pixels?
      if not silent then message,'There aren''t enough valid pixels in the sky annulus.', /con
      goto, BADSTAR
  endif
  skybuf = rotbuf[ sindex[0:nsky-1] ]
  
  
  ;;; --- <TODO><Debug><2013-05-23><Start>
;  simage = rotbuf * 0.0
;  simage[sindex] = rotbuf[sindex]
;  ssidmap = MAKE_ARRAY(3,n_elements(sindex),/DOUBLE,VALUE=0.D)
;  ssidmap[0,*] = (sindex mod nx) + lx 
;  ssidmap[1,*] = (sindex / nx) + ly
;  ssidmap[2,*] = image[LONG(ssidmap[0,*]),LONG(ssidmap[1,*])]
;  ssimage = image * 0.0
;  ssimage[lx:ux,ly:uy] = simage[*,*]
;  print, sqrt(rsq[(103-ly)*nx+(184-lx)]) ;;; the first sky pixel is 184,103
;  s = surface(ssimage)
;  window, 1
;  plot, ssimage[*,yc-15], yrange=[-1.5e-4,1.5e-4], ystyle=1
;  window, 2
;  plot, image[*,yc], yrange=[-1.5e-4,1.5e-4]
;  window, 3
;  plot, skybuf
  ;;; --- <TODO><Debug><2013-05-23><End>
  
  
  if keyword_set(meanback) then $
      meanclip, skybuf, skymod, skysig, CLIPSIG=clipsig, MAXITER=maxiter, CONVERGE_NUM=converge_num else $
...
endelse
 ...
 for k = 0,Naper-1 do begin      ;Find pixels within each aperture
   ...
   if ( edge[i] GE apr[k] ) then begin    ;Does aperture extend outside the image?
     if keyword_set(EXACT) then begin
       mask = dblarr(nx[i],ny[i])
;      good = where( ( x1 LT smallrad[k] ) and (y1 LT smallrad[k] ), Ngood)     ;<correct><20130912><DzLiu> original code
       good = where( (x1^2+y1^2) LT smallrad[k]^2 , Ngood )                     ;<correct><20130912><DzLiu> Use circle rather than box!
       if Ngood GT 0 then mask[good] = 1.0
;      bad = where(  (x1 GT bigrad[k]) or (y1 GT bigrad[k] ))   ;Fix 05-Dec-05
       bad = where(  (x1^2+y1^2) GT bigrad[k]^2 )   ;Fix 05-Dec-05  ;<correct><20130912><DzLiu> Use circle rather than box!
       mask[bad] = -1
       gfract = where(mask EQ 0.0, Nfract) 
       if Nfract GT 0 then mask[gfract] = PIXWT(dx[i],dy[i],apr[k],xx[gfract],yy[gfract]) > 0.0
       ...
       gfract = where(1 - full)
       factor = (area[k] - Nfull ) / total(fractn[gfract],/DOUBLE)
...
;
     ...
     endif else if chk_badpix then begin
     ...
     if not badflux then $ 
                 apmag[k] = total(thisapd*fractn,/DOUBLE) ;Total over irregular aperture
   endif
endfor ;k
  ...
  apmag[g] = apmag[g] - skymod*area[g]  ;Subtract sky from the integrated brightnesses
   ...
   ; <dzliu><2013-09-02> while measuring aperture flux for ideal image and blurry image, we do not consider random photon noise
   IF KEYWORD_SET(NoRandomPhotonNoise) THEN error2[g] = 0.0D
   ; <dzliu><2013-09-02> over
