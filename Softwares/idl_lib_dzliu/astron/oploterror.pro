...
; later, when it is time to deal with the error bar hats).
 ...
 if ~keyword_set(lobar) and ~keyword_set(hibar) then begin
      lobar=1
      hibar=1
...
; Only draw error bars for X values within XCRANGE
    ...
    if (Ng GT 0) and (Ng NE n) then begin  
          ...
          istart = 0L & iend = n-1
    endelse
 if N_elements(ecol) EQ 0 then ecol= cgcolor('opposite')
 if N_elements(ecol) EQ 1 and N_elements(xx) GT 1 then ecol= replicate(ecol,N_elements(xx)) ; <Corrected><20131013><DzLiu>
    
 FOR i = istart, iend, Nskip DO BEGIN

    cgplots, [xx[i],xx[i]], [ylo[i],yhi[i]], LINESTYLE=est, THICK=eth,  $
                                             NOCLIP=noclip, COLOR=ecol[i], ADDCMD=window

    ; Plot X-error bars 
    ;
    if np EQ 4 then $
       cgplots, [xlo[i],xhi[i]], [yy[i],yy[i]], LINESTYLE=est, THICK=eth, $
                                                NOCLIP=noclip, COLOR=ecol[i], ADDCMD=window
    ...
    IF ~keyword_set(nohat) THEN BEGIN
       ...
       exx2 = exx1 + hln
       if lobar then $
          cgplots, [exx1,exx2], [data_low[1,i],data_low[1,i]], LINESTYLE=est, THICK=eth, /DEVICE, $
                                                               NOCLIP=noclip, COLOR=ecol[i], ADDCMD=window
       if hibar then $
          cgplots, [exx1,exx2], [data_hi[1,i],data_hi[1,i]], LINESTYLE=est, THICK=eth, /DEVICE, $
                                                             NOCLIP=noclip, COLOR=ecol[i], ADDCMD = window
;                                          
       IF np EQ 4 THEN BEGIN
          IF (N_elements(hln) EQ 0) THEN hln = !D.Y_VSIZE/100.
             ...
             eyy2 = eyy1 + hln
             if lobar then $
                cgplots, [x_low[0,i],x_low[0,i]], [eyy1,eyy2], LINESTYLE=est, THICK=eth, /DEVICE, $
                                                               NOCLIP=noclip, COLOR=ecol[i], ADDCMD=window
             if hibar then $
                cgplots, [x_hi[0,i],x_hi[0,i]], [eyy1,eyy2], LINESTYLE=est, THICK=eth, /DEVICE, $
                                                             NOCLIP=noclip, COLOR=ecol[i], ADDCMD=window
          ENDIF
       ENDIF
    NOPLOT:
