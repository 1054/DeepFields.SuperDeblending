...
;               Nth value of color.   Colors can also be specified as a string
;               e.g. ('red').   See cgCOLOR() for a list of color names.       
...
;-
  ...
  if N_params() LT 3 then begin
    ...
    if keyword_set( DATA ) then begin 
                ...
                irad =  round( coord[0] )
                ; <TODO><Modified><20140717><DzLIU>
                coord = abs(convert_coord(0,radius,/data,/to_dev) - $
                        convert_coord(0,0,/data,/to_dev)) 
                irad =  round( coord[1] )
                ; <TODO><Modified><20140717><DzLIU><>
   ...
   if keyword_set(data) then begin        ;Convert to device coordinates
        ...
        PRINT, FORMAT='(2F20.12,2F15.3)', xc[0], yc[0], coord[0,0], coord[1,0]
   endif else begin
