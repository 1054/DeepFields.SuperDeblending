PRO CrabContourPlot_v1, Title=Title, XTitle=XTitle, YTitle=YTitle, $
                        ColorbarTitle = ColorbarTitle, ColorbarColor = ColorbarColor, $
                        XArray, YArray, ZArray, Levels=Levels, $
                        BackgroundImage = BackgroundImage, TVScale = TVScale, $
                        XRange = XRange, YRange = YRange, $
                        XTICKINTERVAL = XTICKINTERVAL, YTICKINTERVAL = YTICKINTERVAL, $
                        OverPlot = OverPlot, KeepPlot = KeepPlot, $
                        SaveEPS = SaveEPS
    
    ; Check
    IF N_ELEMENTS(SaveEPS) EQ 0 AND !D.NAME NE 'PS' THEN BEGIN
        MESSAGE, 'CrabContourPlot: Please set SaveEPS! <TODO>'
    ENDIF
...
;<TODO>;Levels = [0.0,Levels]
        Levels = [MIN(ZArray,/NAN),Levels,MAX(ZArray,/NAN)]
    ENDIF
    ...
    ; plot background image if defined
    IF N_ELEMENTS(BackgroundImage) GT 0 THEN BEGIN
        ; http://www.exelisvis.com/docs/CONTOUR_Procedure.html
        ; 
        ; CrabImageQuickPlot, BackgroundImage, Position=[!X.WINDOW[0],!Y.WINDOW[0],!X.WINDOW[1]-!X.WINDOW[0],!Y.WINDOW[1]-!Y.WINDOW[0]]
          CrabImageQuickPlot, BackgroundImage, Position=[!X.WINDOW[0],!Y.WINDOW[0],!X.WINDOW[1],!Y.WINDOW[1]], TVScale=TVScale
        ; 
        ; plot contour frame axis
;        Contour, XTITLE=XTitle, YTITLE=YTitle, TITLE=Title, $
;                 ZArray, XArray, YArray, /NODATA, LEVELS=Levels, /IRREGULAR, $
...
;                 XTICKINTERVAL=XTICKINTERVAL, YTICKINTERVAL=YTICKINTERVAL, $
;                 XThick=1.5, YThick=1.5, $
;                 Thick=1.5, CharSize=2.2, CharThick=4.2, /NOERASE
    ENDIF
    ...
    Contour, $
             XRange=XRange, YRange=YRange, $
             XStyle=XStyle, YStyle=YStyle, $
             ZArray, XArray, YArray, /NOERASE, /OVERPLOT, /CELL_FILL, LEVELS=Levels, /IRREGULAR
             ; TODO: /CELL_FILL /FILL
...
;             C_ORIENTATION=[90], C_LINESTYLE=1, C_COLORS=[cgColor("black"),cgColor("black")]
    ...
    Contour, $
             XRange=XRange, YRange=YRange, $
             XStyle=XStyle, YStyle=YStyle, $
             ZArray, XArray, YArray, /NOERASE, /OVERPLOT, LEVELS=Levels, /IRREGULAR, $
             Color = cgColor("black"), $
    ...
    IF !D.NAME EQ 'PS' THEN BEGIN
        ...
        FOR dl_i=0,N_ELEMENTS(dl_levels)-1 DO BEGIN
            ...
            dl_barChar = ''
            IF ABS(dl_levels[dl_i]) LT 10. AND ABS(dl_levels[dl_i]) GT 0.1 THEN dl_barChar = STRING(FORMAT='(F0.1)',dl_levels[dl_i]) ELSE dl_barChar = STRING(FORMAT='(G0.3)',dl_levels[dl_i])
    ...
    IF NOT KEYWORD_SET(KeepPlot) THEN BEGIN
        DEVICE, /CLOSE
        SET_PLOT, 'X'
