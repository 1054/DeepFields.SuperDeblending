FUNCTION CrabImageFindPeaks, GalaxyImage, MaxPeakNumber=MaxPeakNumber, $
    ...
    IF N_ELEMENTS(MaxPeakNumber) EQ 0 AND N_ELEMENTS(MaxAreaPercent) EQ 0 THEN MaxAreaPercent=0.5 ; defaultly we use MaxAreaPercent to constrain
   ;WHILE CurLoopFlag NE -2 DO BEGIN
    WHILE 1 EQ 1 DO BEGIN
        ...
        IF N_ELEMENTS(MaxAreaPercent) EQ 1 THEN BEGIN
            IF (CurMaxValue-CurMinValue) LE MaxAreaPercent*(TotMaxValue-TotMinValue) THEN BREAK ; MaxAreaPercent=50%
        ENDIF
        IF N_ELEMENTS(MaxPeakNumber) EQ 1 THEN BEGIN
            IF N_ELEMENTS(ListPeaks) GE MaxPeakNumber THEN BREAK
        ENDIF
