function ImageRebin,image,nsout,nlout,total=total,double=double
    ;+
    ; NAME:
    ;   ImageRebin
    ;
    ...
    ;
    ; EXPLANATION:
    ;   ImageRebin is an alternative to CONGRID or REBIN.    Like CONGRID it
    ...
    ;
    ; CALLING SEQUENCE:
    ;   result = ImageRebin( image, nsout, nlout, [ /TOTAL] )
    ;
    ; INPUTS:
    ...
    ;           n = 100/3.1   ; 32.2581
    ;          image_out = ImageRebin(image,n,n)
    ;
    ...
    if N_params() LT 1 then begin
        print,'Syntax = newimage = ImageRebin(image, nsout, nlout, [/TOTAL])'
        return,-1
    endif
    ...
    if (nsout eq long(nsout)) and (nlout eq long(nlout)) then begin
        ...
        if ((nsout mod ns) EQ 0) and ((nlout mod nl) EQ 0) then begin
            ...
            if nl EQ 1 then begin
                if keyword_set(total) then $
                    return,interpolate(image,xindex)*sbox $
                else $
                    return,interpolate(image,xindex)
            endif
            yindex = long(lindgen(nlout)/(nlout/nl))
            if keyword_set(total) then $
                return,interpolate(image,xindex,yindex,/grid)*sbox*lbox $
            else $
    ...
    if nl EQ 1 then begin
        ...
        for i=0L,nsout-1 do begin
            ...
            result[i] = total(image[istart:istop],/NAN) $
                - frac1 * image[istart]  $
                - frac2 * image[istop]
        endfor
    ...
    temp = dblarr(ns,nlout,/NOZERO) ;[DzLiu]
    image = double(image)
    ...
    for i=0L,nlout-1 do begin
        ...
        if istart EQ istop then $
            ...
            temp[0,i] = total(image[*,istart:istop],2,/NAN) $
            - frac1 * image[*,istart] $
            - frac2 * image[*,istop]
    endfor
    temp = transpose(temp)
    ...
    result = dblarr(nlout,nsout,/NOZERO); [DzLiu]
    ...
    for i=0L,nsout-1 do begin
        ...
        ; from istart to rstart and fraction pixel from rstop to istop
        if istart eq istop then $
            result[0,i] = (1.-frac1-frac2)*temp[*,istart] $
        else $
            result[0,i] = total(temp[*,istart:istop],2,/NAN)   $
            - frac1 * temp[*,istart]  $
            - frac2 * temp[*,istop]
    endfor
    
    if keyword_set(total) then result = transpose(result) $
    else result = transpose(result)/(sbox*lbox)
    
    if dtype eq 'DOUBLE' or keyword_set(double) then $
