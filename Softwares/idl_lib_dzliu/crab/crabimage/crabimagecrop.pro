...
; 
    ...
    ENDIF ELSE IF N_ELEMENTS(CentreXY) GT 0 THEN BEGIN
        ...
        ENDIF ELSE BEGIN
            ...
            ;<fixing><20160804>FractiXY = [DOUBLE(CentreXY[0])-(CornerXY[0]+CornerXY[1])/2.0,DOUBLE(CentreXY[1])-(CornerXY[2]+CornerXY[3])/2.0]
            CornerIJ = ROUND(CornerXY)-1
            FractiXY = [DOUBLE(CentreXY[0]-1)-(CornerIJ[0]+CornerIJ[1])/2.0,DOUBLE(CentreXY[1]-1)-(CornerIJ[2]+CornerIJ[3])/2.0] ;<fixing><20160804><TODO>
            ; Width_IJ = CornerIJ[1] - CornerIJ[0]
            ; Hight_IJ = CornerIJ[3] - CornerIJ[2]
        ENDELSE
    ENDIF ELSE BEGIN
    ...
    ;<fixing><20160804>ShiftedXY = [DOUBLE(LX+LEX),DOUBLE(LY+LEY)]
    ShiftedXY = [DOUBLE(LX),DOUBLE(LY)] ;<fixing><20160804><TODO>
    NAXIS = [SX,SY]
    ...
    IF N_ELEMENTS(FITSHeader) GE 1 AND SIZE(FITSHeader,/TNAME) EQ "STRING" THEN BEGIN
        IF SXPAR(FITSHeader,'CRPIX1') NE "" AND SXPAR(FITSHeader,'CRPIX2') NE "" THEN BEGIN
            ...
            SXADDPAR, FITSHeader, 'CRPIX1', CRPIX1-ShiftedXY[0] ;- FractiXY[0]
            SXADDPAR, FITSHeader, 'CRPIX2', CRPIX2-ShiftedXY[1] ;- FractiXY[0]
;            TODO
        ENDIF 
    ENDIF
